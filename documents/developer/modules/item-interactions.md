...
- `src/features/items/InlineQuickAdd.tsx`
...
- `InlineQuickAdd.tsx` holds the TickTick-style one-line capture surface shown at the top of active task pages.
...
- Inline Quick Add captures a task title from the page top without opening a dialog. It uses the same parser and create-capture mutation path as the dialog.
...
- Inline Quick Add appears only on active task pages. Today defaults to a date task for today, Inbox and Upcoming default to Inbox, Deadlines defaults to a deadline task for today, Idea Pool defaults to idea, and Area detail defaults to the current area.
...
- Inline Quick Add does not parse area names from natural language. Area detail keeps the current area unless the user changes the Area option explicitly.
